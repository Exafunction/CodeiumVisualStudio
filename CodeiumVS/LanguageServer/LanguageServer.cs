...
public class LanguageServer
{
    ...
    public async Task SignInAsync()
    {
        ...
        string portalUrl = Package.SettingsPage.EnterpriseMode ? Package.SettingsPage.PortalUrl
    ...
    public async Task PrepareAsync()
    {
        ...
        progDialog.StartWaitDialog("Codeium",
                                   $"Downloading language server v{languageServerVersion}",
                                   "",
                                   null,
                                   $"Codeium: Downloading language server v{languageServerVersion}",
                                   0,
                                   false,
                                   true);

        // the language server is downloaded in a thread so that it doesn't block the UI
        // if we remove `while (webClient.IsBusy)`, the DownloadProgressChanged callback won't be
        // called until VS is closing, not sure how we can fix that without spawning a separate
        // thread
        void ThreadDownloadLanguageServer()
        {
            ...
            Uri url = new(
                $"{extensionBaseUrl}/language-server-v{languageServerVersion}/language_server_windows_x64.exe.gz");
                ...
                ThreadHelper.JoinableTaskFactory
                    .RunAsync(async delegate {
                        ...
                        using FileStream outputStream =
                            new(Package.GetLanguageServerPath(), FileMode.Create);
        ...
        Thread trd = new(new ThreadStart(ThreadDownloadLanguageServer)) { IsBackground = true };
        trd.Start();
    }
    ...
    private async Task StartAsync()
    {
        ...
        string apiUrl = (Package.SettingsPage.ApiUrl.Equals("") ? "https://server.codeium.com"
                                                                : Package.SettingsPage.ApiUrl);
        ...
        if (Package.SettingsPage.EnterpriseMode)
            process.StartInfo.Arguments +=
                $" --enterprise_mode --portal_url {Package.SettingsPage.PortalUrl}";
        ...
        catch (Exception ex)
        {
            ...
            await VS.MessageBox.ShowErrorAsync("Codeium: Failed to start the language server.",
                                               "Please check the output window for more details.");
        }

        string apiKeyFilePath = Package.GetAPIKeyPath();
        if (File.Exists(apiKeyFilePath)) { Metadata.api_key = File.ReadAllText(apiKeyFilePath); }
    ...
    {
        string url =
            $"http://127.0.0.1:{Port}/exa.language_server_pb.LanguageServerService/{command}";
        return await RequestUrlAsync<T>(url, data, cancellationToken);
    }

    public async Task<IList<CompletionItem>?> GetCompletionsAsync(
        string absolutePath, string text, Languages.LangInfo language, int cursorPosition,
        string lineEnding, int tabSize, bool insertSpaces, CancellationToken token)
    {
            ...
            new() { metadata = GetMetadata(),
                    ...
                    editor_options = new() {
                        ...
                        disable_autocomplete_in_comments =
                            !Package.SettingsPage.EnableCommentCompletion,
                    } };
    ...
    {
        return new() { request_id = Metadata.request_id++,
