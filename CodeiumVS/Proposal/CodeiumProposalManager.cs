...
internal class CodeiumProposalManager : ProposalManagerBase
{
    ...
    internal static Task<ProposalManagerBase> TryCreateAsync(ITextView view, CodeiumProposalManagerProvider factory)
    {
        if (view is IWpfTextView wpfTextView)
        {
            return Task.FromResult((ProposalManagerBase)new CodeiumProposalManager(wpfTextView, factory));
        }
    ...
    public override bool TryGetIsProposalPosition(VirtualSnapshotPoint caret, ProposalScenario scenario, char triggerCharacter, ref bool value)
    {
        switch (scenario)
        {
            case ProposalScenario.Return:
                if (caret.Position.GetContainingLine().End == caret.Position)
                    value = true;
                break;
            ...
            case ProposalScenario.TypeChar:
                if (char.IsWhiteSpace(triggerCharacter) && caret.Position.Position >= 2)
                {
                    ...
                    if (LineBreakCharacters.Contains(c) || !char.IsWhiteSpace(c))
                        value = true;
                }
                else
                {
