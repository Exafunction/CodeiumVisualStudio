...
*/            suggestion = new Tuple<String, String[]>(combineSuggestion, suggestionLines);
        ...
        public IEnumerable<ITagSpan<SuggestionTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            if (String.IsNullOrEmpty(line.GetText()))
            {
                lineHeight = view.LineHeight;
            }
            yield return new TagSpan<SuggestionTag>(span, new SuggestionTag(0, 0, lineHeight, 0, height, PositionAffinity.Predecessor, stackPanel, this));
        }
        ...
        void AddSuffixTextBlocks(int start, string line, string userText)
        {
            ...
            if (isTextInsertion && start > line.Length)
            {
                return;
            }
        ...
        public void UpdateAdornment(IWpfTextView view, string userText, int suggestionStart)
        {
            ...
            for (int i = suggestionStart; i < suggestion.Item2.Length; i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    {
                        if (suggestionIndex > 0 && char.IsWhiteSpace(line[suggestionIndex - 1]) && userText.Length > insertionPoint + 1 && !char.IsWhiteSpace(userText[userText.Length - insertionPoint - 1]))
                        {
                            suggestionIndex--;
                        }
                    ...
                    else
                    {
                        ...
                        {
                           String suggestedLine = virtualText.Length > 0 ? virtualText + line.TrimStart() : line;
                           AddSuffixTextBlocks(userText.Length > 0 ? suggestionIndex + offset : 0, suggestedLine, userText);
                        }
                    }
                }
                else
        ...
        public void Update()
        {
            ...
            int suggestionIndex = StringCompare.CheckSuggestion(suggestion.Item1, line, isTextInsertion, insertionPoint);
        ...
        public bool CompleteText()
        {
            ...
            int suggestionLineN = StringCompare.CheckSuggestion(suggestion.Item1, line, isTextInsertion, insertionPoint);
...
#pragma warning restore 649, 169
        ...
        {
            Func<ITagger<T>> sc = delegate () { return new SuggestionTagger((IWpfTextView)textView, buffer) as ITagger<T>; };
            return buffer.Properties.GetOrCreateSingletonProperty<ITagger<T>>(typeof(SuggestionTagger), sc);
        }
