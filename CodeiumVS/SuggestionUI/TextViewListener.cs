...
namespace CodeiumVS
{
    ...
    internal class CodeiumCompletionHandler : IOleCommandTarget, IDisposable
    {
        ...
        public async void GetCompletion()
        {
            ...
            SnapshotPoint? caretPoint = _view.Caret.Position.Point.GetPoint(textBuffer => (!textBuffer.ContentType.IsOfType("projection")), PositionAffinity.Successor);
            ...
            if(list != null && list.Count > 0)
            {
                ...
                List<Tuple<String, String>> suggestions = ParseCompletion(list, text, line, prefix, characterN);
        ...
        List<Tuple<String, String>> ParseCompletion(IList<Packets.CompletionItem> completionItems, string text, string line, string prefix, int cursorPoint)
        {
            ...
            for (int i = 0; i < completionItems.Count; i++)
            {
                ...
                if(completionItem.completionParts.Count == 0)
                {
                    continue;
                }
        ...
        internal CodeiumCompletionHandler(IVsTextView textViewAdapter, ITextView view, TextViewListener provider)
        {
        ...
        private SuggestionTagger GetTagger()
        {
            ...
            {
                return props.GetProperty<SuggestionTagger>(key);
            }
            else
            {
                return null;
        ...
        {
            m_provider.CompletionBroker.DismissAllSessions(_view);
        }
    ...
    internal class TextViewListener : IVsTextViewCreationListener
    {
        ...
        public void VsTextViewCreated(IVsTextView textViewAdapter)
        {
            ...
            Func<CodeiumCompletionHandler> createCommandHandler = delegate () { return new CodeiumCompletionHandler(textViewAdapter, textView, this); };
