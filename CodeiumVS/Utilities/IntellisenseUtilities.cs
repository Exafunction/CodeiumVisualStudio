...
internal static class IntellisenseUtilities
{
    ...
    public static async Task<ITagAggregator<T>> GetTagAggregatorAsync<T>(ITextView textView) where T : ITag
    {
        if (textView.Properties.TryGetProperty(typeof(ITagAggregator<T>), out ITagAggregator<T> tagAggregator))
            return tagAggregator;

        await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
        tagAggregator = MefProvider.Instance.TagAggregatorFactoryService.CreateTagAggregator<T>(textView);
    ...
    // Get or create the tag aggregator, only blocks on the first call
    public static ITagAggregator<T> GetTagAggregator<T>(ITextView textView) where T : ITag
    {
        if (textView.Properties.TryGetProperty(typeof(ITagAggregator<T>), out ITagAggregator<T> tagAggregator))
            return tagAggregator;
        ...
        {
            return await GetTagAggregatorAsync<T>(textView);
        });
    }
}